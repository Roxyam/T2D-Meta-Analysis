...
Data$`E_MEXP_1425.SAT.Ob - C`$TopTab
```
...
Mats_out = glue("{args$outdir}/args$prefix_mats.RData")
MA_out = glue("{args$outdir}/args$prefix_MA.RData")
DF_out = glue("{args$outdir}/args$prefix_DF.RData")
...
for (gene in rownames(sigData)[1]){
  res <- MA[[gene]]
  # Forest plot
  name = glue("{PlotsDir}/{args$prefix}_{gene}forest.svg")
  #svg(name)
  forest(res, 
...
a  + coord_cartesian(ylim = c(0.192,-0.01)) 
