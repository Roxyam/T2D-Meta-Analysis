...
# Resumen del metaanálisis  {.tabset .tabset-fade -}
...
RXcolor <- function(lis, max=50){ 
  ...
  if (!is.null(add)){ret = c(ret, glue("as_chunk('{add}', props = fp_text_default(color = 'black'))"))}
  return(ret)
}
...
```{r echo=T, eval=TRUE, message=FALSE, warning=FALSE}
Dir = "/home/rmoldovan/T2D-Meta-Analysis/Data/OpenTargets"
...
```{r echo=T, eval=TRUE, message=FALSE, warning=FALSE}
DirData = "/home/rmoldovan/T2D-Meta-Analysis/Data/04MA/Obesity"
...
Resumen del metaanálisis en obesidad. 
...
```{r echo=T, eval=TRUE, message=FALSE, warning=FALSE}
Data = data.frame(Contraste = names(Res[[1]]),
...
### Intersección {.tabset .tabset-fade -}   
...
Leyenda = lapply(colnames(Data), function(x) upset_query(group= x,
                                                color="#878785",
                                                fill="#878787"))
...
UpsetPlot <- upset(Data, 
    ...
    # Color de las lineas de interseccion:
...
En este apartado se estudia a causa de las diferencias en el contraste 
SDIO en base al efecto en mujeres (verde) y hombres (azul).
...
Comparación de los perfiles obtenidos con los previamente asociados a la 
obesidad (azul), diabetes de tipo 2 (verde) o ambas, según la información
extraída de Open Targets.
...
# Get table
Tabla = data.frame(Contrastes = names(Res),
  Up = rep("",4), Down = rep("", 4))
ft <- flextable(Tabla)
...
header = data.frame(col_keys = colnames(Tabla),
                    H1 = c("Contraste", "Significancia", "Significancia"),
                    H2 = c("Contraste", "logFC > 0", "logFC < 0")
                    #H2=colnames(Data)
                    )
...
```{r echo=T, eval=TRUE, message=FALSE, warning=FALSE}
DirData = "/home/rmoldovan/T2D-Meta-Analysis/Data/04MA/Diabetes"

f= c("ID/Meta-analysis_1_DF.RData", 
     "IDM/Meta-analysis_2_DF.RData",
     "IDF/Meta-analysis_3_DF.RData",
     "DSID/Meta-analysis_4_DF.RData")
